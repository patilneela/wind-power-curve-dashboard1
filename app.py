...
start_date = pd.to_datetime(st.sidebar.date_input("Start Date", value=max_date - timedelta(days=15)))
end_date = pd.to_datetime(st.sidebar.date_input("End Date", value=max_date)) + pd.Timedelta(days=1)
...
st.subheader(f"{site} | {num_turbines} Turbines | {capacity_per_turbine} MW Each | Total: {round(num_turbines*capacity_per_turbine,2)} MW")
...
def process_turbine(t):
    ...
    return df_t, merged, merged["Deviation_%"].mean(skipna=True)

def generate_comment(dev):
    if dev is None: return "Data not available"
    dev = round(dev, 2)
    if dev < -72: return f"🔴 Dev: {dev}% → Extreme issue (Data unreliable)"
    elif dev < -10: return f"🔴 Dev: {dev}% → Severe underperformance (Blade/Yaw/Dust issue)"
    elif dev < -2: return f"🟠 Dev: {dev}% → Underperformance (Control/availability)"
    elif dev > 72: return f"🟣 Dev: {dev}% → Abnormal high (Sensor/Data issue)"
